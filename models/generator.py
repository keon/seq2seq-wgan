...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg=None, teacher_forcing_ratio=0.4):
        batch_size = src.size(1)
        max_len = trg.size(0)
        vocab_size = self.decoder.output_size
        outputs = Variable(torch.zeros(max_len, batch_size, vocab_size)).cuda()
        contexts = Variable(torch.zeros(max_len, batch_size,
                            self.decoder.hidden_size)).cuda()
